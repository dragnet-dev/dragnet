...
func TestGenGolden(t *testing.T) {
	files := []struct{ input, output string }{
		{"testdata/input_network.sigma.yaml", "testdata/golden_network.eql"},
		{"testdata/input_process.sigma.yaml", "testdata/golden_process.eql"},
		{"testdata/input_file.sigma.yaml", "testdata/golden_file.eql"},
	}
	b := elastic.New()
	for _, f := range files {
