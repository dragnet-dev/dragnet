...
func TestGenGolden(t *testing.T) {
	files := []struct{ input, output string }{
		{"testdata/input_network.sigma.yaml", "testdata/golden_network.spl"},
		{"testdata/input_process.sigma.yaml", "testdata/golden_process.spl"},
		{"testdata/input_file.sigma.yaml", "testdata/golden_file.spl"},
	}
	b := splunk.New()
	for _, f := range files {
