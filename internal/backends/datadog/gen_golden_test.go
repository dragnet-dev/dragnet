...
func TestGenGolden(t *testing.T) {
	files := []struct{ input, output string }{
		{"testdata/input_network.sigma.yaml", "testdata/golden_network.json"},
		{"testdata/input_process.sigma.yaml", "testdata/golden_process.json"},
		{"testdata/input_file.sigma.yaml", "testdata/golden_file.json"},
	}
	b := datadog.New()
	for _, f := range files {
