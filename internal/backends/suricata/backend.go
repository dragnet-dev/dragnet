...
// Backend compiles Sigma network rules to Suricata IDS rules.
// Non-network rules emit a comment placeholder — Suricata only handles network traffic.
type Backend struct{}

func New() *Backend { return &Backend{} }

func (b *Backend) Name() string            { return "suricata" }
...
func buildRules(rule *sigmaRule, detection map[string]interface{}) (string, error) {
	...
	// Only network_connection and dns_query categories map naturally to Suricata.
	category := strings.ToLower(rule.LogSource.Category)
	if category != "network_connection" && category != "dns_query" {
		return fmt.Sprintf(
			"# Suricata: rule %q (ID: %s) targets %q — no network rule generated\n",
	...
	// Walk detection and collect domains and IPs.
	...
	for _, domain := range domains {
		sb.WriteString(fmt.Sprintf(
			`alert dns any any -> any any (msg:"%s [%s]"; dns.query; content:"%s"; nocase; classtype:%s; sid:%d; rev:1;)`+"\n",
...
func deriveSID(sigmaID string) uint32 {
	...
	return n%9000000 + 1000000 // keep in [1000000, 9999999]
}
