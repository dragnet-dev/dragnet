...
func TestGenGolden(t *testing.T) {
	files := []struct{ input, output string }{
		{"testdata/input_network.sigma.yaml", "testdata/golden_network.yaral"},
		{"testdata/input_process.sigma.yaml", "testdata/golden_process.yaral"},
		{"testdata/input_file.sigma.yaml", "testdata/golden_file.yaral"},
	}
	b := chronicle.New()
	for _, f := range files {
